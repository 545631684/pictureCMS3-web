<template>
  <div>
    ...
    <div class="imgs shangchuan" style="width: 40%;">
      <el-upload multiple style="" :limit="20" ref="newengineeringFile" accept=".rar,.zip" class="upload-demo newfiles" :action="action + '?id=8'" :on-remove="handleRemoveEngineering" :on-change="obtainImgSrc" :on-exceed="limitNum">
      	<el-button size="small" type="primary">点击上传压缩包</el-button>
      	<div slot="tip" class="el-upload__tip">只能上传.rar,.zip格式文件，文件大小不要超过1GB</div>
      </el-upload>
    	<el-upload :multiple="true" :file-list="fileList" :limit="20" ref="engineeringFile" accept=".rar,.zip" class="upload-demo" :action="action + '?id=8'" :on-remove="handleRemoveEngineering" :on-change="obtainImgSrc" :on-exceed="limitNum">
    		<!-- <el-button size="small" type="primary">点击上传工程文件</el-button>
    		<div slot="tip" class="el-upload__tip">只能上传.rar,.zip格式文件，文件大小不要超过1GB</div> -->
    	</el-upload>
    	<el-alert title="提示" description="📣一次最多上传20个工程文件，超出部分会自动剔除" type="info" show-icon style="width: 600px; margin: 20px 0;"></el-alert>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    inject: ['reload'],
    name: 'engineeringUpdate',
    props: ['articleEngineering'], // 接收父组件传值的方法
    components: {
    },
    data() {
      return {
    ...
    watch: {
      fileALL: function(newQuestion, oldQuestion) {
        // 关闭前 给父组件传递值
        this.$emit('engineeringData', this.fileALL)
        if(this.fileALL.length === 0){
          this.$refs.newengineeringFile.clearFiles()
          this.uploadFiles.splice(0, this.uploadFiles.length)
          this.fileList.splice(0, this.fileList.length)
        }
      },
      articleEngineering: function(newQuestion, oldQuestion) {
        let _this = this, num = []
        if(this.articleEngineering.length !== 0){
          _this.articleEngineering.find(obj => {
            	num[num.length] = {
          			name: obj.file.name,
          			url: obj.file.url
          		}
          })
          _this.fileALL = this.articleEngineering
          _this.fileList = num
        }
      },
    },
    methods: {
      ...
      limitNum (file, fileList) {
      	if (this.fileALL.length > 20) {
      		this.$alert('最多上传20个工程文件，你已超出限制！', '警告', {
      ...
      // 删除上传的文件Engineering
      handleRemoveEngineering(file, fileList) {
      	...
      	this.engineering.find((obj, index) => {
      		if (obj !== undefined) {
      			if (obj.file.url.indexOf(fileUrl) !== -1) {
      				// 删除文章Engineering的
      				_this.engineering.splice(index,1)
      			}
      		}
      	})
