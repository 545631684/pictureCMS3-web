  ...
  <el-container style="margin: 50px 0px;" v-loading="loading2">
  	...
  	<el-footer style="min-height: 200px;height: auto !important; padding-bottom: 50px;">
  		...
  		<div class="imgs shangchuan" style="width: 40%;">
  			<el-upload :multiple="true" :file-list="fileListPdf" :limit="20" ref="aiFile" accept=".xlsx,.xls" class="upload-demo" :action="action + '?id=8'" :on-remove="handleRemovePdf" :on-change="obtainImgSrc" :on-exceed="limitNum">
    ...
    methods: {
      ...
      handleRemovePdf(file, fileList) {
        ...
        if (file.response !== undefined) fileUrl = file.response.data.file
      	this.article.excel.find((obj, index) => {
      		if (obj !== undefined) {
      			if (obj.file.url === fileUrl) {
      				// 删除文章Pdf的
      				_this.article.excel.splice(index,1)
      				// 统一删除变量里添加删除的文件路径
      ...
      submitImg() {
        let _this = this
      	...
      	} else if(this.article.excel.length === 0) {
    	...
    	this.getAdminArticle({mId: this.$route.params.id})
    	  .then((response) => {
            ...
            _this.article2 = JSON.parse(_this.article2)
    	      ...
    	      var num = []
    	       if (response.data.typeFile === 'excel') {
    	      	response.data.excel.find(obj => {
