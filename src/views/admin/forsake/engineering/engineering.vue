<template>
  <el-container style="margin: 50px 0px;">
  	...
  	<el-footer style="min-height: 200px;height: auto !important; padding-bottom: 50px;">
  		...
  		<div class="imgs">
  			<el-upload :limit="130" ref="fliesImg" :multiple="true" accept=".jpg,.png,.gif" :action="action + '?id=1'" list-type="picture-card" :on-preview="handlePictureCardPreview" :on-remove="handleRemove" :on-change="obtengineeringImgSrc" :on-exceed="limitNumImg">
  	...
  	<el-footer style="min-height: 200px;height: auto !important; padding-bottom: 50px;">
  		...
  		<div class="imgs" style="width: 40%;">
  			<el-upload multiple style="" :limit="20" ref="videoFile" accept=".rar,.zip" class="upload-demo" :action="action + '?id=8'" :on-remove="handleRemove3" :on-change="obtengineeringImgSrc" :on-exceed="limitNum">
  				<el-button size="small" type="primary">点击上传工程文件</el-button>
  				<div slot="tip" class="el-upload__tip">只能上传.rar,.zip格式文件，文件大小不要超过1GB</div>
  			</el-upload>
  			<el-alert title="提示" description="※上传第一个文件时会自动获取文件名称并填写到标题。※一次最多上传20个工程文件，超出部分会自动剔除" type="info" show-icon style="width: 600px; margin: 20px 0;"></el-alert>
  		</div>
  	</el-footer>
    ...
    methods: {
      ...
      obtengineeringImgSrc(file, fileList) {
      	...
      	if(file.response !== undefined) {
      		if(file.response.data.type === '1') {
      		...
      		} else if(file.response.data.type === '8') {
      			...
      			// 设置标题为第一个上传的文件的名称
      			if (this.title.length === 0) {
      				this.title = this.engineering[0].file.name.split(".")[0]
      				this.titleRepeat()
      			}
      		}
      ...
      submitImg() {
      	...
      	} else if(this.engineering.length === 0 ) {
      		this.$alert('请添加需要上传的工程文件', '警告', {
      			confirmButtonText: '确定'
      		})
      	} else {
          let _this = this
          this.loading = true
          this.articleAdd({uId:this.$store.state.admin.adminInfo.uId, typeFile:'engineering', typeid:this.$store.getters.getUserTypesId(this.typeImg).tid, projectid:this.$store.getters.getUserProjectsId(this.projectImg).pid, detailsid:this.$store.getters.getUserMinTypeId(this.minTypeImg).did, title:this.title.replace(/\s+/g," "), keyword:this.dynamicTags.toString(), describe:this.describe.replace(/\s+/g," "), img:this.imgCrss, psd:'[]', video:'[]', ai:this.ai, pdf:'[]', word:'[]', excel:'[]', engineering:this.engineering})
            .then((response) => {
              if(response.code === 200) {
                _this.setOperationInfo({_this:_this, type:31, article:{uId:_this.$store.state.admin.adminInfo.uId, typeFile:'engineering', typeid:_this.$store.getters.getUserTypesId(_this.typeImg).tid, projectid:_this.$store.getters.getUserProjectsId(_this.projectImg).pid, detailsid:_this.$store.getters.getUserMinTypeId(_this.minTypeImg).did, title:_this.title.replace(/\s+/g," "), keyword:_this.dynamicTags.toString(), describe:_this.describe.replace(/\s+/g," "), img:_this.imgCrss, psd:'[]', video:'[]', ai:_this.ai, pdf:'[]', word:'[]', excel:'[]', engineering:_this.engineering}})
