  ...
  <el-container style="margin: 50px 0px;" v-loading="loading2">
  	...
  	<el-footer style="min-height: 200px;height: auto !important; padding-bottom: 50px;">
  		...
  		<div class="imgs shangchuan" style="width: 40%;">
  			<el-upload :multiple="true" :file-list="fileListAi" :limit="20" ref="aiFile" accept=".ai" class="upload-demo" :action="action + '?id=7'" :on-remove="handleRemoveAi" :on-change="obtainImgSrc" :on-exceed="limitNum">
  ...
  export default {
    ...
    methods: {
      ...
      handleRemoveAi(file, fileList) {
      	// 特别说明下：fileListAi在修改时只做读取之用，不做写入（多个文件写入时会报错status值为null），上传组件会自己显示修改上传的文件
      	let _this = this, fileUrl = ''
      	...
      	this.article.video.find((obj, index) => {
      		if (obj !== undefined) {
      			if (obj.dataAi.url === fileUrl) {
      				...
      				// 统一删除变量里添加删除的文件路径
      				_this.deleteFiles.push(obj.dataAi.url)
      			}
      		}
      	})
      },
      ...
      submitImg() {
        let _this = this
      	...
      	}  else if(this.article.ai.length === 0) {
    	...
    	this.getAdminArticle({mId: this.$route.params.id})
    	  .then((response) => {
            ...
            _this.article2 = JSON.parse(_this.article2)
    	      ...
    	      var num = []
    	       if (response.data.typeFile === 'ai') {
    	      	response.data.ai.find(obj => {
    	          	_this.fileListAi.push({
