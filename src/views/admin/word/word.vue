    ...
    methods: {
      	...
      	if(file.response !== undefined) {
      		if(file.response.data.type === '8') {
      			...
      			if (this.title.length === 0) {
      				this.title = this.word[0].file.name.split(".")[0]
      				this.titleRepeat()
      			}
      		}
      ...
      submitImg() {
      	if (this.$store.state.admin.adminInfo.nickname === this.$store.state.admin.adminInfo.userName) {
      		...
      		this.$alert('请添加需要上传的word文件', '警告', {
      			confirmButtonText: '确定'
      		})
      	} else {
          let _this = this
          this.loading = true
          this.articleAdd({uId:this.$store.state.admin.adminInfo.uId, typeFile:'word', typeid:this.$store.getters.getUserTypesId(this.typeImg).tid, projectid:this.$store.getters.getUserProjectsId(this.projectImg).pid, detailsid:this.$store.getters.getUserMinTypeId(this.minTypeImg).did, title:this.title.replace(/\s+/g," "), keyword:this.dynamicTags.toString(), describe:this.describe.replace(/\s+/g," "), img:'', psd:'', video:'', ai:'', pdf:'', word:this.word, excel:'', engineering:''})
            .then((response) => {
              if(response.code === 200) {
                _this.setOperationInfo({_this:_this, type:31, article:{uId:_this.$store.state.admin.adminInfo.uId, typeFile:'word', typeid:_this.$store.getters.getUserTypesId(_this.typeImg).tid, projectid:_this.$store.getters.getUserProjectsId(_this.projectImg).pid, detailsid:_this.$store.getters.getUserMinTypeId(_this.minTypeImg).did, title:_this.title.replace(/\s+/g," "), keyword:_this.dynamicTags.toString(), describe:_this.describe.replace(/\s+/g," "), img:'', psd:'', video:'', ai:'', pdf:'', word:_this.word, excel:'', engineering:''}})
